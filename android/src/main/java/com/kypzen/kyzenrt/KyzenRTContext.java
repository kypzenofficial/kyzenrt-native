...
public class KyzenRTContext {
  public static final String NAME = "KyzenRT";
  private static final String TAG = "KyzenRT";
  ...
  public KyzenRTContext(ReactApplicationContext reactContext) {
    ...
    try {
      ...
      if (isArm64V8a()) {
        if (hasDotProd && hasI8mm && hasHexagon && hasAdreno) {
          if (tryLoadLibrary("kyzenrt_jni_v8_2_dotprod_i8mm_hexagon_opencl")) {
            jniLoaded = true;
            loadedLib = "kyzenrt_jni_v8_2_dotprod_i8mm_hexagon_opencl";
          }
        }

        if (!jniLoaded && hasDotProd && hasI8mm) {
          if (tryLoadLibrary("kyzenrt_jni_v8_2_dotprod_i8mm")) {
            jniLoaded = true;
            loadedLib = "kyzenrt_jni_v8_2_dotprod_i8mm";
          }
        }

        if (!jniLoaded && hasDotProd) {
          if (tryLoadLibrary("kyzenrt_jni_v8_2_dotprod")) {
            jniLoaded = true;
            loadedLib = "kyzenrt_jni_v8_2_dotprod";
          }
        }

        if (!jniLoaded && hasI8mm) {
          if (tryLoadLibrary("kyzenrt_jni_v8_2_i8mm")) {
            jniLoaded = true;
            loadedLib = "kyzenrt_jni_v8_2_i8mm";
          }
        }

        if (!jniLoaded && hasFp16) {
          if (tryLoadLibrary("kyzenrt_jni_v8_2")) {
            jniLoaded = true;
            loadedLib = "kyzenrt_jni_v8_2";
          }
        }

        if (!jniLoaded) {
          if (tryLoadLibrary("kyzenrt_jni_v8")) {
            jniLoaded = true;
            loadedLib = "kyzenrt_jni_v8";
          }
        }
      } else if (isX86_64()) {
        if (tryLoadLibrary("kyzenrt_jni_x86_64")) {
          jniLoaded = true;
          loadedLib = "kyzenrt_jni_x86_64";
        }
      } else {
        if (tryLoadLibrary("kyzenrt_jni")) {
          jniLoaded = true;
          loadedLib = "kyzenrt_jni";
        }
      }

      if (!jniLoaded) {
        // Fallback to generic JNI
        System.loadLibrary("kyzenrt_jni");
        loadedLib = "kyzenrt_jni";
      }

      System.loadLibrary("kyzenrt");
